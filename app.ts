...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  if (name === llmRouterTool.name) {
    return await llmRouterHandler(args as { question: string });
  } else if (name === dbTool.name) {
    return await dbToolHandler(args as { question: string });
  } else if (name === webSearchTool.name) {
    return await webSearchHandler(args as { query: string });
  }
